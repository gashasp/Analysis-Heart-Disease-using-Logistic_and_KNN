...
*-Accuracy : 0.7705 --> 77.0% model to correctly guess the target (Heart Disease / Not Heart Disease).*

*-Sensitivity (Recall) : 0.8000 --> 80% from all the positive actual data, capable proportion of model to guess right.*

*-Specificity : 0.7143 --> 71.4% from all the negative actual data, capable proportion of model to guess right.*

*-Pos Pred (Precision) : 0.8421 --> 84.2% from all the prediction result, capable model to correctly guess the positive class.*
...
**From  business case, I want to choose patients with Heart Disease, for that I choose a model with high value Pred (Precision) and Accuracy. I choose a model with a high value Post Pred (Precision) and Accuracy because I want the model be able to predict accurately and don't want wrong prediction of patients with Heart Disease or FP (Prediction correct but Actual wrong) is better than FN (Prediction wrong but Actual correct).**
